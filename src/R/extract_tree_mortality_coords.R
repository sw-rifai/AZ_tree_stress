...
fwrite(tab2, file = "outputs/AZ_tree_mort_coords.csv")


# tab
# 
# dat$created_date[1]
# st_drivers() %>% pull(name) %>% sort
# 
# st_write(dat %>% select(all_of(sel)), 
#   dsn = "outputs/AZ_tree_mortality_coords.shp", 
#   append = F)
# 
# st_write(dat %>% select(all_of(sel)), 
#   dsn = "outputs/AZ_tree_mortality_coords.csv", 
#   append = F)
# 
# 
# tmp <- fread("outputs/AZ_tree_mortality_coords.csv")
# tmp
